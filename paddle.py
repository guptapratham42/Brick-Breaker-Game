...
class paddle:
    ...
    def render(self):
        ...
        self.padd_mat=([[ Fore.BLUE + "F" for i in range(global_var.paddle_length)] for j in range(2)])
        self.temp=global_var.paddle_mid
        for i in range(2):
            for j in range(global_var.paddle_length):
                global_var.display.grid[global_var.height-2+i][global_var.paddle_mid-int(global_var.paddle_length/2)+j]=self.padd_mat[i][j]
    def clear(self):
        for i in range(2):
            for j in range(global_var.width):
                global_var.display.grid[global_var.height-2+i][j]=' '
